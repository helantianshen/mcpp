...
开发态实现与验证均已完成，开发内容位于 `mcpp` 分支；音频授权确认和发布包资源注入按用户要求延后。
...
实现无阻塞。发布前需要取得音频许可、将三份 MP3 注入各平台发布包，并补发布包内资源存在性冒烟测试。

# 推荐下一步

由维护者试听并审阅 `mcpp` 分支；准备发布时联系 https://github.com/whitefirer，然后完善 release 打包与各平台实听。
